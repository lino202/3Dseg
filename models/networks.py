...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, is2D, input_nc=None,
        ...
        downconv = nn.Conv3d(input_nc, inner_nc, kernel_size=kernel_size, stride=stride, padding=padding, bias=use_bias)
        ...
        if outermost:
            upconv = nn.ConvTranspose3d(inner_nc * 2, outer_nc,kernel_size=kernel_size, stride=stride, padding=padding)
            down   = [downconv]
            up     = [uprelu, upconv, nn.Tanh()]
            model  = down + [submodule] + up
        elif innermost:
            upconv = nn.ConvTranspose3d(inner_nc, outer_nc,kernel_size=kernel_size, stride=stride, padding=padding, bias=use_bias)
            down   = [downrelu, downconv]
            up     = [uprelu, upconv, upnorm]
            model  = down + up
        else:
            upconv = nn.ConvTranspose3d(inner_nc * 2, outer_nc,kernel_size=kernel_size, stride=stride, padding=padding, bias=use_bias)
